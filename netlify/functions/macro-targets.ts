...
import { dayTypes, optionalNonNegativeInteger, requireEnum, targetGoals } from "./_shared/validation";
  ...
  try {
    ...
    if (event.httpMethod === "GET") {
      ...
      const rows = await sql`
        ...
        where user_id = ${userId}
          and (${goal ?? null}::text is null or target_goal = ${goal ?? null})
    ...
    if (event.httpMethod === "PUT") {
      ...
      const [target] = await sql`
        ...
        values (
          ${userId}, ${payload.target_goal}, ${payload.day_type}, ${payload.kcal_min}, ${payload.kcal_max}, ${payload.protein_min},
          ${payload.carbs_min}, ${payload.carbs_max}, ${payload.fat_min}, ${payload.fat_max}
        )
        on conflict (user_id, target_goal, day_type)
        do update set
...
function validateTarget(fields: Record<string, unknown>) {
  return {
    target_goal: requireEnum(fields, "target_goal", targetGoals),
